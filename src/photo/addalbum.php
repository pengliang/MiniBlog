...
function checkaddcatform(id)
{
    ...
    if(checkaddinput(id)&&checkbytem('sde',200,2,"您输入的相册简介太长，请保持在100字以内。"))
	{
 		...
 		parent.$.post("result.php?action=addalbum", {albumname: name,albumdesc: desc}, function(response)
		{
 			if(response.status=="success")
	 		{ 
 				parent.addCallBack(response.album);
 			}
		 },"json");
...
<table cellspacing="6" cellpadding="1" border="0" class="formhd">
	<tbody>
		<tr>
			...
			<td><input type="text" onfocus="hidErr(1);"
				onchange="checkaddinput('spAlbumName')" style="width: 290px;"
				maxlength="49" id="spAlbumName" name="spAlbumName" /> <span
				class="gray">(必填)</span></td>
		</tr>
		...
		<tr>
			<td> </td>
			<td class="formsb"><input type="submit" value=" 创建 " name="tj" />&nbsp;&nbsp;&nbsp;&nbsp;<a
